...
class Jbig3_Auth_AuthAdapter implements Zend_Auth_Adapter_Interface {
     ...
     * @param $identityColumn string           
     * @param $credentialColumn string           
     * @param $credentialTreatment string           
     * @return void
     */
    ...
    /**
     * setEm() - set the Doctrine Entity Manager
     *
     * @param $em \Doctrine\ORM\EntityManager           
     */
     ...
     * @param $useMd5 boolean           
     * @return My_Auth_Adapter_Doctrine2 Provides a fluent interface
     */
    public function setCredential($credential, $useMd5 = false){
        if($useMd5)
     ...
     * making sure
     * that this adapter was indeed setup properly with all required peices of
     * information.
     *
     ...
     * @throws Zend_Auth_Adapter_Exception - when a invalid select object is
     *         encoutered
     * @return array
     */
    protected function _performQuery(Doctrine\ORM\Query $query){
        try {
            ...
            throw new Zend_Auth_Adapter_Exception(
                    'The supplied parameters to \Doctrine\ORM\EntityManager failed to ' .
 ...
 * result set is indeed a record that matched the identity provided to this
 * adapter.
 *
 * @param $resultIdentities array           
 * @return Zend_Auth_Result
 */
...
/**
 * _authenticateCreateAuthResult() - This method creates a Zend_Auth_Result
 * object
 * from the information that has been collected during the authenticate()
 * attempt.
 *
 * @return Zend_Auth_Result
 */
...
return new Zend_Auth_Result($this->_authenticateResultInfo['code'], 
        $this->_authenticateResultInfo['identity'], $this->_authenticateResultInfo['messages']);
}
